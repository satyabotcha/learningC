...
/*
Find all lines matching the pattern string, return its last (or rightmost occurence) string index
*/
...
int main()
{
    ...
    char pattern[] = "rld"; // pattern to search for
    int found = 0;
...
int strindex(char line[], char pattern[])
{
    ...
    */
    ...
    int i, j, k;
    int index = -1; // default is not matched
    ...
    for (i = 0; line[i] != '\0'; i++)
    {
        ...
        if (k > 0 && pattern[k] == '\0')
        {
            index = i; // set current matched pattern to index -> we update the index to reflect the latest matched index
        }
    }
    return index;
}
